...
import { mcq, distractors, hasField, sameMeaning } from "../engine/quiz";
import { isPluralOnly, pluralFormOf } from "../engine/nounForms";
import { pluralDistractors, participleDistractors, presentDistractors } from "../engine/formDistractors";
import {
...
export const CATEGORIES = [
  {
    ...
    modes: [
      { id: "translation", label: "Translation", eligible: (it) => hasField(it.e), build: (it, p) => mcq(it.w, "What does this mean?", it.e, distractors(p, it.e, "e", 3, sameMeaning(it))) },
      // A plural-only noun (die Eltern) has no gender to ask about.
      { id: "article", label: "Article", eligible: (it) => ["der", "die", "das"].includes(it.a) && !isPluralOnly(it), build: (it) => ({ prompt: it.w.replace(/^(der|die|das)\s+/i, ""), sub: "der, die or das?", answer: it.a, options: ["der", "die", "das"] }) },
      // `p` doubles as a note ("(kein Plural)", "(immer Plural)"), which is
      // not a plural to quiz. The wrong options are wrong plurals of the same
      // noun (die Tischen, die Tischer), not other nouns' plurals.
      { id: "plural", label: "Plural", eligible: (it) => !!pluralFormOf(it), build: (it, p) => mcq(it.w, "What is the plural form?", it.p.trim(), distractors(p, it.p.trim(), (x) => pluralFormOf(x) && x.p.trim(), 3, null, pluralDistractors(it))) },
      { id: "german", label: "EN → DE", eligible: (it) => hasField(it.w), build: (it, p) => mcq(it.e, "German noun (with article)?", it.w, distractors(p, it.w, "w", 3, sameMeaning(it)), "en") },
    ],
  },
  {
    ...
    modes: [
      { id: "translation", label: "Translation", eligible: (it) => hasField(it.e), build: (it, p) => mcq(it.w, "What does this mean?", it.e, distractors(p, it.e, "e", 3, sameMeaning(it))) },
      { id: "partizip2", label: "Partizip II", eligible: (it) => hasField(it.p2), build: (it, p) => mcq(it.w, "What is the Partizip II?", it.p2, distractors(p, it.p2, "p2", 3, null, participleDistractors(it))) },
      ...
      { id: "present_er", label: "Präsens (er)", eligible: (it) => !!it.pr && hasField(it.pr.er), build: (it, p) => mcq(it.w, "er/sie/es form (Präsens)?", it.pr.er, distractors(p, it.pr.er, (x) => x.pr && x.pr.er, 3, null, presentDistractors(it))) },
    ],
  },
  {
  ...
  },
  {
    ...
    modes: [
      { id: "translation", label: "DE → EN", eligible: (it) => hasField(it.e), build: (it, p) => mcq(it.w, "What does this phrase mean?", it.e, distractors(p, it.e, "e", 3, sameMeaning(it))) },
      { id: "german", label: "EN → DE", eligible: (it) => hasField(it.w), build: (it, p) => mcq(it.e, "Say this in German", it.w, distractors(p, it.w, "w", 3, sameMeaning(it)), "en") },
    ],
  },
  {
    ...
    modes: [
      { id: "translation", label: "DE → EN", eligible: (it) => hasField(it.e), build: (it, p) => mcq(it.w, "What does this word mean?", it.e, distractors(p, it.e, "e", 3, sameMeaning(it))) },
      { id: "german", label: "EN → DE", eligible: (it) => hasField(it.w), build: (it, p) => mcq(it.e, "Say this in German", it.w, distractors(p, it.w, "w", 3, sameMeaning(it)), "en") },
    ],
  },
];
